...
allocate_scheduling_group_specific_data(scheduling_group sg, unsigned long key_id, const std::shared_ptr<scheduling_group_key_config>& cfg) {
...
reactor::init_new_scheduling_group_key(scheduling_group_key key, scheduling_group_key_config cfg) {
    return with_lock(_scheduling_group_keys_mutex, [this, key, cfg] {
        ...
        auto cfgp = std::make_shared<scheduling_group_key_config>(std::move(cfg));
